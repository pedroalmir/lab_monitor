...
public class Firebase {
	...
	/**
	 * Overloaded constructor for cases where you need to prevent adding the
	 * json extension to the url.
	 * 
	 * @param baseUrl
	 * @param useJsonExtension
	 *            include the json extension to the URL?
	 ...
	 */
	public String getAsString(String path) throws FirebaseException, UnsupportedEncodingException {
		// make the request
		String url = this.buildFullUrlFromRelativePath(path);
		HttpGet request = new HttpGet(url);
		HttpResponse httpResponse = this.makeRequest(request);
		...
		// process the response
		return this.processResponseAsString(FirebaseRestMethod.GET, httpResponse);
	}
	...
	public String patchAsString(String path, String jsonData) throws UnsupportedEncodingException, FirebaseException {
		// make the request
		String url = this.buildFullUrlFromRelativePath(path);
		HttpPatch request = new HttpPatch(url);
		request.setEntity(this.buildEntityFromJsonData(jsonData));
		...
		// process the response
		return this.processResponseAsString(FirebaseRestMethod.PATCH, httpResponse);
	}
	...
	public String putAsString(String path, String jsonData) throws FirebaseException, UnsupportedEncodingException {
		...
		// process the response
		return this.processResponseAsString(FirebaseRestMethod.PUT, httpResponse);
	}
	...
	private FirebaseResponse processResponse(FirebaseRestMethod method, HttpResponse httpResponse) throws FirebaseException {
		...
		switch (method) {
			case DELETE:
				if (httpResponse.getStatusLine().getStatusCode() == 204 && "No Content".equalsIgnoreCase(httpResponse.getStatusLine().getReasonPhrase())) {
					success = true;
				}
				break;
			case PATCH:
			...
			case POST:
			case GET:
				if (httpResponse.getStatusLine().getStatusCode() == 200 && "OK".equalsIgnoreCase(httpResponse.getStatusLine().getReasonPhrase())) {
					success = true;
				}
				break;
			default:
	 ...
	 * @throws FirebaseException
	 */
	private String processResponseAsString(FirebaseRestMethod method, HttpResponse httpResponse) throws FirebaseException {
		// sanity-checks
		if (method == null) {
			String msg = "method cannot be null";
			LOGGER.error(msg);
